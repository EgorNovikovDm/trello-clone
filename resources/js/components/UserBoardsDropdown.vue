<template>
    <div>
        ...
        <DropDownMenu :show="showBoards" @closed="showBoards = false">
            ...
            <router-link :to="{name: 'board', params: {id: board.id}}" v-for="board in userBoards" :key="board.id" :class="[`bg-${board.color}-100`]"
                 class="rounded-sm opacity-100 hover:opacity-75 text-gray-700 font-bold cursor-pointer flex mb-1" @click.native="showBoards = false">
