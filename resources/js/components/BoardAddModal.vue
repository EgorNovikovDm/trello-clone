            ...
            <div class="flex">
                ...
                <div>
                    <div class="flex justify-between mb-2" v-for="(row, i) in colorGrid" :key="i">
                        <BoardColor v-for="(c,i) in row" :key="i" :color="c" :activeColor="color"
                                    @changed="color = $event"></BoardColor>
                    </div>
                </div>
            </div>
            <div class="mt-4">
                <button @click="addBoard" :disabled="cannotCrate"
                        class="rounded-sm py-2 px-4 text-white hover:opacity-75 cursor-pointer disabled:opacity-25"
                        :class="[colors[color]]">Create
                </button>
            </div>
        </Modal>
    </div>
...
export default {
    ...
    computed: {
        ...
        colorGrid: () => colorGrid,
        cannotCrate() {
            return this.title == null || this.title.lenght == 0;
        }
    },
    methods: {
        addBoard() {
